...
async function updateGreenBackground(){const wanted=imageKey(),img=$('#greenBaseImage'),stage=$('#greenStage');let src='';try{const blob=await imageGet(wanted);if(blob)src=URL.createObjectURL(blob)}catch{}src=src||localStorage.getItem(wanted)||defaultImage();if(wanted!==imageKey())return;if(src){img.src=src;stage.classList.add('has-photo')}else{img.removeAttribute('src');stage.classList.remove('has-photo')}}
