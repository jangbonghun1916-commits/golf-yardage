...
async function updateGreenBackground(){const wanted=greenImageKey(),img=$('#greenBaseImage'),stage=$('#greenStage');let src='';try{const blob=await imageGet(wanted);if(blob)src=URL.createObjectURL(blob)}catch{}if(wanted!==greenImageKey())return;if(src){img.src=src;stage.classList.add('has-photo')}else{img.removeAttribute('src');stage.classList.remove('has-photo')}}
